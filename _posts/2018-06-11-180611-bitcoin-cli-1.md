...
- deprecated된 파라미터 account는 'lable'로 변경되었다. '-deprecatedrpc = accounts'로 실행하는 경우에는 이전 버전과의 호환성을 고려한다.
...
#### 'label' RPC 메서드는 다음 기능적 차이점을 제외하고 'account' 기능을 반영합니다.
- 레이블은 주소 수신뿐만 아니라 모든 주소에서 설정할 수 있다. 이 기능은 이전에는 GUI를 통해서만 가능했다.
- 레이블은 setlabel RPC 메소드를 사용하여 모든 주소를 재 지정하여 삭제할 수 있다.
- 레이블에서 트랜잭션을 전송하거나 트랜잭션이 전송 된 레이블을 결정할 수는 없다.
- 'label'은 'balance'가 없다.

  
## Here are the changes to RPC methods:
|  <center>Deprecated Method</center>	|  <center>New Method</center> 	|  <center>Notes</center>  |
| :---: | :---: | :---: |
| getaccount	| getaddressinfo	| `getaddressinfo`는 계정 이름 대신 주소 정보가있는 json 객체를 문자열로 반환합니다. |
| getaccountaddress	| getlabeladdress | 만약 'label'이 이미 존재하지 않는다면, `getlabeladdress`는 디폴트로 에러를 던지지만, 기존 응용 프로그램과의 호환성을위한 강제 옵션(force option)을 제공합니다. |
| getaddressesbyaccount	| getaddressesbylabel | 
`getaddressesbylabel`은 문자열 목록 대신 key와 같은 주소를 가진 json 객체를 반환합니다. |
...
- move, sendfrom는 no replacement이다.
- 대체되는 건??
...
## Changed Method
- listtransaction() api 를 쓰지말고, listunspent() API 를 이용해서 주소의 tx 목록을 얻는다.
...
## 중요한 것  
getblock, getblockcount, getblockhash, getinfo, getrawtransaction, getaccount, getaddressesbyaccount, getbalance, getnewaddress, gettransaction, listtransactions, move, sendfrom, sendmany, sendtoaddress, multisig, validateaddress, listunspent  
...
## getaccount ( DEPRECATED. 향후 account자체를 아예 deprecated 함.)
- `getaccount "address"`
...
- arguments : 
    ...
    * "inclued_watchonly" (bool, optional, default=false) : 잔액계산과 세부사항에 시계전용주소(?)를 포함 할지의 여부.
...
1. bitcoin-cli getbalance kw : 발란스 생성
2. bitcoin-cli listtransactions kw : 리스트 보기
