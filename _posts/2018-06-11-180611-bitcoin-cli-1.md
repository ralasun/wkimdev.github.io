...
- 0.15v
- bitcoincore에 공시된 최신 버전 0.16.1v

# deprecated되는 'account' API ('label' and 'account' APIs for wallet).
- 새 **'label' API**가 wallet에 도입되어, 'account'API를 대체하게 될 것.
- deprecated된 파라미터 account는 'lable'로 변경된다.
- 'account'는 '-deprecatedrpc = accounts' 인자로서 V0.17에서 계속 사용될 수 있으며 **V0.18에서 완전히 제거될 예정이다.**
...
1. 최초 getbalance 확인.
2. getnewaddress wkim 로 새 주소 생성
3. getbalance wkim 새 계좌 잔고 확인.
3. sendtoaddress wkim-address-value 2 송금 실행 (리턴값은 txid)
4. listunspent 0 로 '미확정된' 트랜잭션 확인.  
(발생한 txid, address에 대한 confirmations = 0, 시간이 좀 지난 후 전송이 되면 confirmations = 1로 바뀐다.)  
5. getbalance 송금 후 전체 잔액확인 (기존 잔액 - 송금액).  
6. getbalance wkim 수신 금액 확인.  
7. listunspent 0 로 , txid에 대한 confirmations값이 바뀐걸 확인.  
8. listtransactions 를 통해 방금 이뤄진 트랜잭션 내역을 확인.  
