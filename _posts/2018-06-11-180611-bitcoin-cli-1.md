...
getInfo();
getTransaction(tran_id : string);
getBalance();
getBalanceType(btype:string);
getAddress(pkey:string,tag:string);
isValid(addr:string);
send(receivers : any);
getGasFee(tran_id:string);
getConfirmation(txid : string);
updateInsertConfirmation(txid : string);
cronSendUserBalanceToSendOutAccout(fromaddress:string,gas:number,gasp:number);
...
- returns : account ""은 파라미터가 없는것(디폴트) 과 다르다(?). 서버의 전체 잔액은 기본계정의 잔액과 다를 수 있다.  
            ...
            * account = specified (DEPRECATED) 계좌의 잔액 리턴. (
