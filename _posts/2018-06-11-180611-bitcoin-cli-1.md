...
--- | ---
| addmultisigaddress | `account` 파라미터를 `label` 파라미터로 변경. '-deprecatedrpc = accounts'를 실행하면 이전 버전과의 호환성을 고려한다.  (ex) `addmultisigaddress nrequired ["key",...] ( "account" )` ===> ( "account" ) 파라미터를 `("label")`로 변경. |
| getnewaddress | `account` 파라미터를 `label` 파라미터로 변경. '-deprecatedrpc = accounts'를 실행하면 이전 버전과의 호환성을 고려한다.  (ex) `getnewaddress ( "account" )` ===> ( "account" ) 파라미터를 `("label")`로 변경. |
| listunspent | 새 `label` 필드를 반환. '-deprecatedrpc = accounts'로 실행하면 이전 버전과의 호환성을 위해 계정 필드가 반환. |
| listtransactions | `account` 파라미터를 `dummy` 로 변경. dummy 매개변수는 `*`(string) 사용(-deprecatedrpc = accounts 를 사용하지 않을 경우). (ex) `$ bitcoin-cli listtransactions "*"` |
